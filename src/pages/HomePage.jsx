...
export default function HomePage() {
      ...
      try {
        ...
        if (active) {
          ...
          setRegistrationReopenDate(
            settings.registration_reopen_date ?? '',
          );
        }
      } catch (error) {
        console.error(
  ...
  const registrationAutomaticallyOpen =
    !registrationOpen
    && registrationReopenDate
    && todayKey >= registrationReopenDate;

  const registrationsAreOpen =
    registrationOpen || registrationAutomaticallyOpen;

  let registrationStatus = '';

  if (registrationsAreOpen) {
    registrationStatus = registrationSeason
      ? `SAISON ${registrationSeason}`
      : '';
  } else if (registrationReopenDate) {
    const [year, month, day] =
      registrationReopenDate.split('-');

    registrationStatus =
      `OUVERTURE LE ${day}/${month}/${year}`;
  } else {
    registrationStatus = 'FERMÉES';
  }
    ...
    <main className="home-v2">
      ...
      <section className="home-v2-hero">
        ...
        <Link
          ...
          aria-disabled={!registrationsAreOpen}
          onClick={(event) => {
            if (!registrationsAreOpen) {
